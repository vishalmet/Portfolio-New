...
const Navbar = () => {
    ...
    return (
        ...
        <div className="bg-transparent top-0 z-10 w-full fixed backdrop-filter backdrop-blur-lg bg-opacity-20 pb-3 pt-2 flex justify-between items-center " style={{ boxShadow: '0px 2px 5px rgba(0, 0, 0, 0.1)' }}>
            ...
            {/* <div className={`lg:hidden fixed inset-0 bg-gray-900 bg-opacity-50 z-50 ${isOpen ? 'block' : 'hidden'}`} onClick={closeMenu}></div> */}
            <div className={`lg:hidden absolute inset-y-0 z-50 left-0 transform transition duration-300 ease-in-out bg-white text-black backdrop-filter backdrop-blur-lg h-screen w-64 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
